...
def ScrapeMatchList(user_id):
    offset = ''
    match_ids = set()
    n = 0
    while n < 5:
        if offset == '':
            with request.urlopen('https://pubg.op.gg/api/users/{}/matches/recent?server=na&queue_size=4&mode=fpp'.format(user_id,offset)) as url:
                data = json.loads(url.read().decode())
        else:
            with request.urlopen('https://pubg.op.gg/api/users/{}/matches/recent?server=na&queue_size=4&mode=fpp&after={}'.format(user_id, offset)) as url:
                data = json.loads(url.read().decode())
...
def ScrapeMatchData(match_id):
    name_id_set = set()
    with request.urlopen('https://pubg.op.gg/api/matches/{}/deaths'.format(match_id)) as url:
        data = json.loads(url.read().decode())
